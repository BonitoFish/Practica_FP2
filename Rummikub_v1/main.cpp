...
tFicha robar(tBolsa& bolsa) {
	...
	if (bolsa.fichas[fila_rand][col_rand].valor != -1) {	// Devuelve la ficha si la encuentra en una posici�n random
		...
		// Implementaci�n mediante b�squeda circular
...
void mostrarSeries(tSoporte soporte) {
	int num[13];	// Lista n�meros que ya se ha escogido para no repetir
	int tam_num = 0;
	...
	for (int i = 0; i < soporte.num_fichas; i++) {
		...
		if (tam_num == 0) {	// Este es primer caso para un primer valor (solo se eval�a una vez)
			...
			if (!duplicado) {	// Si el n�mero no se ha repetido, entonces insertar en num para no repetir la pr�xima vez
				num[tam_num] = soporte.lista_ficha[i].valor;
				tam_num++;
			}
			else {	// Se pasa a la siguiente iteraci�n si ya se hab�a recorrido
				continue;
			}
		}

		serie[0] = soporte.lista_ficha[i];	// Suponemos que el primer elemento siempre ser� el del �ndice i
		tam_serie++;
		...
		// Imprime la serie si supera el tama�o m�nimo
		if (tam_serie >= 3) {
...
void mostrarEscaleras(tSoporte soporte) {
	...
	for (int i = 0; i < soporte.num_fichas; i++) {	// Se itera sobre cada uno de los elementos del soporte
		...
		if (tam_num == 0) {
			...
			// Recorro mi lista de n�meros para ver si se ha escogido previamente para formar una Escalera
		...
		escalera[0] = soporte.lista_ficha[i];	// Se supone que en cada iteraci�n el primer elem es el del �ndice i
		int tam_escalera = 1;
		...
		while (j != k) {	// Recorrido circular 
			if (soporte.lista_ficha[j].valor == elem.valor + 1 && soporte.lista_ficha[j].color == elem.color) {
				...
				tam_escalera++;
				elem = soporte.lista_ficha[j];	// Si existe elemento consecutivo empezar la b�squeda desde all�
				k = j;
			}
...
bool existeSerie(tSoporte soporte) {
	// Pr�cticamente id�ntico que mostrarSeries()
	for (int i = 0; i < soporte.num_fichas; i++) {
		tFicha serie[4];
		serie[0] = soporte.lista_ficha[i];
		int cont_serie = 1; // N�mero de elementos de la serie a devolver
		for (int j = i + 1; j < soporte.num_fichas; j++) {	// Recorro para buscar elementos de la serie a devolver
			if (soporte.lista_ficha[j].valor == serie[0].valor) {
				bool mismo_color = false;
...
bool existeEscalera(tSoporte soporte) {
	// Pr�cticamente id�ntico que mostrarEscaleras()
	for (int i = 0; i < soporte.num_fichas; i++) {
		tFicha escalera[13];
...
bool ponerFicha(tJugada& jugada, tFicha ficha) {
	...
	if (esSerie(jugada)) {	// Llamo a la funci�n para comprobar si tras insertar la ficha sigue siendo una serie
...
bool jugar(tTablero& tablero, tSoporte& soporte) {
	...
	// Si ha metido m�s de una Ficha en la Jugada
	if (jugada.num_fichas_jugada > 1) {
		cout << "Jugada:  ";
		mostrar(jugada);

		// si la jugada es v�lida (= 0), a�adir la nueva Jugada en el Tablero
...
int menor(const tSoportes soportes) {
	...
	for (int i = 0; i < soportes.num_jug; i++) {
		...
		if (menor == -1) {	// Solo entra una vez al if para establecer un valor inicial v�lido
			menor = acum;
			ret = i;
		}
		else if (acum < menor) {
